...
def test_allocate_env_avoids_live_port(tmp_path: Path):
    ...
    try:
        config = ProjectConfig(env={"WEB_PORT": EnvSpec(allocate="port", minimum=occupied, maximum=occupied + 1)})
        env = allocate_env(
            config=config,
            manifest=Manifest(),
            name="my-feature",
...
def test_remove_worktree_and_branch_deletes_both(tmp_path: Path):
    ...
    branches = subprocess.check_output(["git", "branch", "--list", "feature/my-feature"], cwd=tmp_path, text=True)
